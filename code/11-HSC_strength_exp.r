...
load("mecom_var_sankaran/data/hememap/interaction_name_gene_info.rda")
load("mecom_var_sankaran/data/hememap/01022021-heme_shortest_paths_geoActivate_info.rda")
...
# interaction_gene_name_group <- sapply(interaction_gene_name_group, unique)
interaction_hsc_exp_group <- sapply(interaction_gene_name_group, function(x) expr_mat_cpm[match(x, tss250[, 4]), "HSC"] )
m <- sapply(interaction_hsc_exp_group, median, na.rm=T)  %>% rev() %>% barplot(., col=rev(as.character(jdb_palette("brewer_spectra", type="continuous"))[time_num:1]), ylim=c(0, 20))
xx <- rep(0, time_num)
for (i in 1:time_num){
